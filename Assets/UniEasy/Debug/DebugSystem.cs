...
namespace UniEasy.Console
{
	public class DebugSystem : SystemBehaviour
	{
		...
		private BoolReactiveProperty isLogEnabled = new BoolReactiveProperty (true);
		private BoolReactiveProperty showOnUGUI = new BoolReactiveProperty (true);
		private ReactiveCommand recordCommand = new ReactiveCommand ();
		...
		public bool IsLogEnabled {
			get {
				return isLogEnabled.Value;
			}
			set {
				isLogEnabled.Value = value;
			}
		}
		...
		public override void Setup ()
		{
			...
			DebugWriter.OnAdd ().Subscribe (writer => {

				if (writer.HasKey ("isLogEnabled")) {
					IsLogEnabled = writer.Get<bool> ("isLogEnabled");
				}
				if (writer.HasKey ("showOnUGUI")) {
					ShowOnUGUI = writer.Get<bool> ("showOnUGUI");
				}
				...
				// We cannot auto add a new layer when debug happened in every platform,
				...
				Debugger.BeforeCheckLayerInEditorEvent += (layerName) => {
					if (!DebugMask.ToList ().Select (mask => mask.layerName).Contains (layerName)) {
						DebugMask.AddMask (true, layerName);
						Refresh ();
				...
				group.OnAdd ().Subscribe (entity => {
					...
					viewEntities.OnAdd ().Subscribe (viewEntity => {
						...
						debugView.collapseToggle.OnPointerClickAsObservable ().Subscribe (_ => {
							...
							writer.Set ("collapse", debugView.Collapse);
						}).AddTo (this.Disposer).AddTo (debugCanvas.Disposer).AddTo (debugView.Disposer);
						...
						debugView.warningToggle.OnPointerClickAsObservable ().Subscribe (_ => {
							...
							writer.Set ("warning", debugView.Warning);
						}).AddTo (this.Disposer).AddTo (debugCanvas.Disposer).AddTo (debugView.Disposer);
						...
						debugView.errorToggle.OnPointerClickAsObservable ().Subscribe (_ => {
							...
							writer.Set ("error", debugView.Error);
						}).AddTo (this.Disposer).AddTo (debugCanvas.Disposer).AddTo (debugView.Disposer);
						...
						debugView.collapseToggle.isOn = writer.HasKey ("collapse") ? writer.Get<bool> ("collapse") : true;
						debugView.logToggle.isOn = writer.HasKey ("log") ? writer.Get<bool> ("log") : true;
						debugView.warningToggle.isOn = writer.HasKey ("warning") ? writer.Get<bool> ("warning") : true;
						debugView.errorToggle.isOn = writer.HasKey ("error") ? writer.Get<bool> ("error") : true;

						debugView.panel.gameObject.SetActive (false);
					}).AddTo (this.Disposer).AddTo (debugCanvas.Disposer).AddTo (writer.Disposer);
				}).AddTo (this.Disposer).AddTo (writer.Disposer);
			}).AddTo (this.Disposer);
		}
		...
		public void Refresh ()
		{
			...
			Debugger.IsLogEnabled = this.IsLogEnabled;
